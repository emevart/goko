...
// (в main.ts — ctx.shutdown), и Realtime не живёт в комнате без человека. Срок меньше emptyTimeout
// комнаты в game-server (300 с) и не зависит от того, как LiveKit считает агента в departure_timeout.

import { type Clock, realClock } from './clock.ts';

// Минута: перезагрузка страницы и короткий обрыв сети укладываются с запасом.
