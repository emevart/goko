...
- Доска: столбцы `A B C D E F G H J K L M N` (без `I`), строки `1–13` снизу; `board` — строка длины `size*size` из `.`, `B`, `W`, индекс `(row - 1) * size + col`, `col` от `A = 0`. Разбор координат принимает `D4`, `d4`, `D 4`, кириллические двойники `А→A, В→B, С→C, Е→E, Н→H, К→K, М→M, О→O, Р→P, Т→T`; отвергает `I` и выход за размер ошибкой `invalid_coord`.
...
`[OK]` Задача 1 выполнена: коммиты `050af24` и `d25f1b2`. Круг правок по итогам
ревью расширил её сверх текста плана, и в этих местах прав код, а не план:

- `toIndex` и `formatCoord` бросают `InvalidCoordError` на точке вне доски;
  `fromIndex` и `indexToCoord` — на индексе вне `[0, size*size)`. В плане они
  описаны без проверок.
- Карта кириллических двойников — десять букв (`А В С Е Н К М О Р Т`), и каждая
  закреплена тестом; `Д` в карту не входит намеренно, на неё есть негативный тест.
- Класс символов в регулярке нормализации строится из ключей карты, а не задан
  вторым списком.
- `'pass'` и `'пас'` сверяются с нормализованным текстом, поэтому `'П А С'` и
  `'па-с'` тоже пас.
- `normalizeCoordText` покрыт своим `describe`.
